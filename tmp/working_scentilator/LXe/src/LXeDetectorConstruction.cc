...
: fLXe_mt(nullptr), fMPTPStyrene(nullptr)
{
  ...
  fLXe = Ej200 = fAl = fAir = fVacuum = fGlass = nullptr;
...
void LXeDetectorConstruction::DefineMaterials(){
  ...
  Ej200_mt->AddConstProperty("SCINTILLATIONYIELD",5000./MeV);
  ...
  assert(sizeof(glass_RIND) == sizeof(PhotonEnergy));
  G4double glass_AbsLength[]={420.*cm,420.*cm,420.*cm};
  assert(sizeof(glass_AbsLength) == sizeof(PhotonEnergy));
  G4MaterialPropertiesTable *glass_mt = new G4MaterialPropertiesTable();
  glass_mt->AddProperty("ABSLENGTH",PhotonEnergy,glass_AbsLength,NUMENTRIES);
  glass_mt->AddProperty("RINDEX",PhotonEnergy,glass_RIND,NUMENTRIES);
...
G4VPhysicalVolume* LXeDetectorConstruction::Construct(){
  ...
  if (fMainVolumeOn)
  {
    // ROTATION MATRIX FOR WORLD
    G4RotationMatrix *rotMat;
    // rotMat = new G4RotationMatrix();
    // rotMat->rotateX(90 * deg);
    rotMat = NULL;
    fMainVolume = new LXeMainVolume(rotMat, G4ThreeVector(), fExperimentalHall_log, false, 0, this);
  }
...
void LXeDetectorConstruction::SetDefaults() {
  ...
  fOuterRadius_pmt = (0.99 * fScint_z) / 2.;
