                 ...
                 "housing",pMotherLogical,pMany,pCopyNo),fConstructor(c)
{
  ...
  // fPmt = new G4Tubs("pmt_tube",innerRadius_p/t,spanningAngle_pmt);
  
  G4double distFromCorner = (fScint_x < fScint_z ? fScint_x : fScint_z) / 8.;// + height_pmt / 2.; // just something
  G4double pmt_width = distFromCorner * 2.;

  fPmt = new G4Box("pmt_box", pmt_width, (fScint_y) / 2.,
                   height_pmt);
  ...
  fPhotocath = new G4Box("photocath_box", pmt_width, (fScint_y) / 2.,
                         height_pmt / 2.);
  ...
  rm->rotateY(135 * deg);
  offset = G4ThreeVector(-(sqrt(2) * (distFromCorner)), 0, -(sqrt(2) * (distFromCorner)));
  new G4PVPlacement(rm, G4ThreeVector(fScint_x / 2., 0, fScint_z / 2.) + offset, fPmt_log, "pmt",
                    fScint_log, false, k++);
  fPmtPositions.push_back(G4ThreeVector(fScint_x / 2., 0, fScint_z / 2.) + offset);
  ...
  fNx = 2; fNy = 1; fNz = 0; // trick SD to recognize 4 pmts
...
void LXeMainVolume::CopyValues(){
  ...
  // fSphereOn=false; // disable sphere
