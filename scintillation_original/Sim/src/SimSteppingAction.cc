...
//
//
/// \file optical/Sim/src/SimSteppingAction.cc
/// \brief Implementation of the SimSteppingAction class
//
//
...
SimSteppingAction::SimSteppingAction(SimEventAction* ea)
  : fOneStepPrimaries(false),
    fEventAction(ea)
{
  fSteppingMessenger = new SimSteppingMessenger(this);
...
void SimSteppingAction::UserSteppingAction(const G4Step * theStep){

  G4Track* theTrack = theStep->GetTrack();

  if ( theTrack->GetCurrentStepNumber() == 1 ) fExpectedNextStatus = Undefined;
 
  SimUserTrackInformation* trackInformation
    =(SimUserTrackInformation*)theTrack->GetUserInformation();
    ...
    if(thePostPoint->GetStepStatus()==fGeomBoundary){
      if(fExpectedNextStatus==StepTooSmall){
        if(boundaryStatus!=StepTooSmall){
          ...
          ed << "SimSteppingAction::UserSteppingAction(): "
                << "No reallocation step after reflection!"
                << G4endl;
          G4Exception("SimSteppingAction::UserSteppingAction()", "SimExpl01",
          FatalException,ed,
      ...
      case Detection: //Note, this assumes that the volume causing detection
        ...
        SimPMTSD* pmtSD = (SimPMTSD*)SDman->FindSensitiveDetector(sdName);
